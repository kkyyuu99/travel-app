...
## 12.8. v25 — 비상정보 카드 + 1일치 공유

### 사용자
> "ㄱㄱ" — v24 직후, 다음 라운드 진행 요청.

### 결정 (도쿄 D-3에 진짜 valuable한 것 2개)
1. **🆘 비상정보** — 사고/도난/응급 시 1순위 연락처. 평소엔 안 보지만 사고 시 1순위. 오프라인 보장.
2. **↗ 1일치 공유** — 가족 카톡으로 "오늘 일정 이거야" 한 번에 보내기.

### 작업 — 🆘 비상정보 ([js/emergency.js](js/emergency.js))
새 모듈 `window.EMERGENCY_INFO` + `getEmergencyForTrip(trip)`:

- **12개국**: JP / FR / ES / PT / IT / CH / AT / HU / CZ / GB / GI / SI
  - 각 국가별: 응급/경찰/소방 번호, 한국 대사관·영사관 (전화·주소·24h emergency), 현지 팁
  - EU는 통합 112 + 국가별 세부 번호 둘 다 표시
- **🇰🇷 한국 공통**:
  - 외교부 영사 콜센터 24h `+82-2-3210-0404`
  - 해외안전여행 `0404.go.kr` 링크
  - 13개 카드 분실신고 (현대·신한·KB·삼성·하나·롯데·BC·NH·우리·씨티·트래블월렛·VISA·MC 글로벌)
  - 도난·분실 시 보험 청구 절차 팁

UI ([index.html](index.html) `openEmergencyInfo`):
- 트립 메뉴 "🆘 비상정보" → 모달
- 트립의 `countryCode` / `countryCodes`로 자동 매칭
- 모든 전화번호 `tel:` 링크 (폰에서 한 탭으로 전화)
- 응급 번호는 빨간색 강조

### 작업 — ↗ 1일치 공유 ([index.html](index.html) `shareCurrentDay`)
- Day 헤더 우측 상단 작은 ↗ 아이콘 버튼
- `buildDayShareText(day)` → 텍스트 포맷:
  ```
  📅 DAY 1 · 2026-05-25
  🎯 시부야 & 롯폰기
  도착 첫날 — 현대카드 쉐어라운지 + 야경 + 곤파치

  09:50 ✈️ 나리타 공항 T3 도착 — 이스타항공 ZE605
  11:00 🚄 케이세이 스카이라이너 — 나리타 T2 → 닛포리 → 시부야
  ...

  — 🗼 도쿄 여행
  ```
- `navigator.share()` 우선 (모바일 시스템 공유 시트 → 카톡/메시지 선택)
- 미지원 또는 실패 시 `navigator.clipboard.writeText()` fallback + 토스트 "📋 복사됨"
- 사용자 취소(AbortError)는 silent

### 검증 (모바일 375×812)
- 모달 2 섹션 (🇯🇵 일본 + 🇰🇷 한국 공통) ✅
- 22개 `tel:` 링크 ✅
- 119/110 강조, 대사관 전화·24h emergency 표시 ✅
- 13개 카드 분실신고 grid ✅
- 1일치 공유 텍스트 정상 포맷 ✅
- 공유 버튼 34×34 우측 상단, 보임 ✅
- 콘솔 에러 0
